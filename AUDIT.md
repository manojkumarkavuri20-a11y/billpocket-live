...
`parseLooseStatementLine` (`app.js:1731`) matches amounts with `/[-(]?\s*[£$€₹]?\s*\d[\d,]*\.\d{2}\)?/g`. The mandatory `\.\d{2}` means a plain-text/PDF line whose amount has **no decimals** (e.g. `15 Jun 2026  NETFLIX  100`) matches nothing, so the row is silently discarded (`app.js:1732`). CSV/OFX/QIF use column mapping and are unaffected. **Remediation: add a currency-symbol-prefixed alternative that allows whole amounts (`[£$€₹]\s*\d[\d,]*(?:\.\d{1,2})?`) while keeping the original strict-decimal branch — so `£100` / `£1,200` are recovered without misreading ISO-date hyphens, sort codes, or bare years as amounts.**
